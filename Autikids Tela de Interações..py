...
def b1():
    s = pyttsx3.init()
    data = f"Olá! Meu nome é Luiz Hilário. Qual seu nome?"
    s.say(data)
    s.runAndWait()
...
imgApresentacoes = PhotoImage(file='botão_apresentações.gif')
b_Apresentacoes = Button(janela, image=imgApresentacoes, bg='orange', border=0, activebackground='orange', command=b1)
b_Apresentacoes.grid(column=0, row=1, sticky='we', padx=35)
